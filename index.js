...
async function run() {
  try {
    ...
    app.get('/users/admin/:email',verifyToken, async(req, res)=>{
      ...
      // checking the owner of token
      if(email !== req.decoded.email){
        return res.status(403).send({message: 'forbidden access'})
      }
      const query = {email: email}
      const user = await userCollection.findOne(query)
      // cheking who is admin
      let admin = false
      if(user){
    ...
    app.post('/users', async(req, res)=>{
      ...
      // you can do this many ways (1. email unique, 2.upsert 3.simple checking->isExist)
    ...
    app.get('/carts', async(req, res)=>{
      ...
      const result = await cartCollection.find(queryUser).toArray()
      res.send(result)
    })
    ...
    // effiecient way --> using aggregate pipeline
